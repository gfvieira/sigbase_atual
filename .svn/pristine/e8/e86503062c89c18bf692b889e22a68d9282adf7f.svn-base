...
public class Finaly50 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (check == true) {
                oficina.setStatus("FINALIZADO");
                check = daooficina.altStatusEnd(oficina, usuario);
                if (check == true) {
                    oficina.setIp("127.0.0.1");
                    oficina.setMove("SIGBASE || STATUS ALTERADO PARA: FINALIZADO");
                    check = daooficina.insereMov(oficina);
                    if (check == true) {
                        ...
                        RequestDispatcher rd = request.getRequestDispatcher("/sigbase/503.html");
                        rd.forward(request, response);
                    }
                } else {
                    RequestDispatcher rd = request.getRequestDispatcher("/sigbase/503.html");
